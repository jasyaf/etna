...
public abstract class SMTPAppenderBase<E> extends AppenderBase<E> {
  ...
  // @CRACK 2014-2-13 2014年春节期间出现了发邮件线程卡死的问题，经查是SMTP一直处于等待状态所导致，这里加入超时设置防止再出问题
  /** 连接SMTP服务器超时时间，单位ms */
  private long coTimeout = 60 * CoreConstants.MILLIS_IN_ONE_SECOND;
  /** 发送时间超时，单位ms */
  private long timeout = 60 * CoreConstants.MILLIS_IN_ONE_SECOND;
  // @CRACK end
  ...
  private Session buildSessionFromProperties() {
    ...
    // @CRACK begin 加入连接超时和发送超时 @2014-02-10
    props.put("mail.smtp.connectiontimeout", String.valueOf(coTimeout));
  ...
  protected void sendBuffer(CyclicBuffer<E> cb, E lastEventObject) {
    ...
    try {
      ...
      // @CRACK begin 2013-8-6 做了一些修改，通过日志发送的邮件采用html格式，并通过<tt>强制等宽、不换行，方便在各大邮箱查看
      String contentType = "text/html"; // 原来是layout.getContentType()现改为统一是html
      String content = "<!DOCTYPE html PUBLIC \"-//W3C//DTD XHTML 1.0 Transitional//EN\" \"http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd\">"
          + "<html xmlns=\"http://www.w3.org/1999/xhtml\">"
          + "<head></head><body><tt><div style=\"white-space:nowrap;\">"
          + StringTools.escapeHtml(sbuf.toString())
          + "</div></tt></body></html>";
      if (ContentTypeUtil.isTextual(contentType)) {
        part.setText(content, charsetEncoding, ContentTypeUtil.getSubType(contentType));
      } else {
